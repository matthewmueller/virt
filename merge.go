...
func (f *mergedFS) Open(path string) (fs.File, error) {
...
func (n *notExists) Error() string {
	if len(n.errors) == 0 {
		return fmt.Sprintf("virt: open %q\n%s", n.path, fs.ErrNotExist)
	}
	return fmt.Sprintf("virt: open %q\n%s", n.path, errors.Join(n.errors...))
}
...
func (f *mergedFS) mergeDir(path string, dirs []dir) (fs.File, error) {
	// Initialize merged
	merged := &mergeDir{
		name:    baseName(path),
	...
	for _, dir := range dirs {
		...
		if !ok {
			return nil, fmt.Errorf("virt: directories doesn't implement ReadDirFile")
		}
